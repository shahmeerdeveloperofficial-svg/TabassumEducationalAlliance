...
const renderBlock = (block, index) => {
  const renderChildren = (block) => {
    ...
    case "br":
      return <br key={index}/>;
    case "h1":
      return <h1 className="mt-3 sm:mt-4 mb-2 leading-[1.1] font-berlin tracking-wide font-bold text-3xl sm:text-4xl text-slate-900" key={index}>{renderChildren(block)}</h1>;
    case "h2":
      return <h2 className="mt-2 sm:mt-3 mb-2 leading-[1.1] font-berlin tracking-wide font-semibold text-2xl sm:text-3xl text-slate-900" key={index}>{renderChildren(block)}</h2>;
    case "h3":
      return <h3 className="sm:mt-2 leading-snug font-semibold text-xl sm:text-2xl text-slate-900" key={index}>{renderChildren(block)}</h3>;
    case "h4":
    ...
    case "p":
      return <p className="text-base sm:text-lg leading-relaxed text-slate-800" key={index}>{renderChildren(block)}</p>;
    case "small":
